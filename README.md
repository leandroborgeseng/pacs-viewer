...
- `docker-compose.yml` — Postgres, Orthanc, API, Web *(dev local)*.
- [`docker-compose.coolify.yml`](docker-compose.coolify.yml) + [`.env.coolify.example`](.env.coolify.example) — pilha **produção** para **Coolify** (Docker Compose).
- [`docs/coolify.md`](docs/coolify.md) — passo a passo de deploy no Coolify.
...
Deploy self-hosted num único recurso Compose: use [`docker-compose.coolify.yml`](docker-compose.coolify.yml), variáveis em [`.env.coolify.example`](.env.coolify.example) e o guia [`docs/coolify.md`](docs/coolify.md).

Resumo: dois domínios HTTPS (`web` = portal, `api` = backend), **`NEXT_PUBLIC_API_URL` no build da Web**, **`WEB_ORIGIN` na API**, **`SKIP_DB_SEED=1`** em produção, **`TRUST_PROXY_HOPS=1`**, healthchecks `/healthz` (web) e `/health` (api). Build da Web exige **~6–8 GB RAM** (OHIF).
