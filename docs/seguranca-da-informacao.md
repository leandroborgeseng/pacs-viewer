...
Documento de referência para alinhar com equipe clínica / DPO / TI. As sugestões marcadas como *futuras* ainda não estão implementadas; volte a este arquivo quando for priorizar trabalho.
...
| Área | Notas |
|------|--------|
| Autenticação | JWT; senhas com bcrypt; erro de login genérico ("Credenciais inválidas"); **rate limit próprio no login**. |
...
| Auditoria REST | Interceptor grava operações sensíveis `POST/PATCH/PUT/DELETE` (exc. login já auditado à parte e **exclui** tráfego massivo `/dicomweb`). |
...
- **`POST /api/auth/login`**: limite **mais apertado que o global**, por IP (`@nestjs/throttler` no handler), *padrão* **20 tentativas / 60 s** — `AUTH_LOGIN_THROTTLE_LIMIT`, `AUTH_LOGIN_THROTTLE_TTL_MS` em `api/.env.example`.
- Atrás de reverse proxy: **`TRUST_PROXY_HOPS`** em `main.ts` para `req.ip` / throttle usarem `X-Forwarded-For` (ex.: Railway: `1`).
- *Ainda futuro*: throttle ou bloqueio também **por e‑mail**; CAPTCHA após N falhas; monitorização de IPs anômalos.

### 2. Armazenamento do JWT no navegador (XSS)

- Se o token vive em `localStorage`, qualquer XSS pode exfiltrar sessão.
- *Mitigações*: CSP rigorosa, atualização de dependências, evitar HTML inseguro; *médio prazo*: cookies **httpOnly** + **SameSite** (com desenho BFF ou equivalente).

### 3. Segredos e integração PACS

- Rotação periódica de `JWT_SECRET`, `LAUDO_SEAL_*`, credenciais Orthanc.
- Credenciais Orthanc podem estar na BD (sem criptografia de aplicação) — limitar quem é `ADMIN`, restringir ACL no Orthanc, rede privada entre API e PACS.

### 4. Auditoria de acesso a dados clínicos (leitura)

- O interceptor **não** registra cada GET ao proxy DICOMweb (volume OHIF).
...
- *Verificar* se um reverse proxy (Railway, Nginx, etc.) injeta HSTS, `X-Content-Type-Options`, etc.; ou usar **Helmet** na Nest se a API for exposta diretamente.

### 6. LGPD / dados de saúde

- Registro de atividades de tratamento, base legal, minimização de dados nas listagens.
...
- Política de divulgação responsável de vulnerabilidades (arquivo `SECURITY.md` à parte, se desejarem).
...
## Performance (conexão indireta)

Com muitos usuários simultâneos, o desenho atual inclui cache de catálogo PACS, cache de allowlist RBAC no proxy e throttle mais alto no `dicomweb`. Se surgirem problemas, rever o commit `perf(api): …` e documentação em `.env.example` (`STUDIES_CATALOG_CACHE_MS`, `ACCESS_ALLOWLIST_CACHE_MS`).
...
- Atualize esta data quando o conteúdo mudar de forma substancial.
